\documentclass[11pt, aspectratio=169]{beamer}
\usetheme{Madrid}
\usecolortheme{default}
\setbeamertemplate{navigation symbols}{}
\setbeamertemplate{footline}[page number]

% Packages
\usepackage[]{inputenc}
\usepackage[english]{babel}
\usepackage{booktabs}
\usepackage{graphicx}
\usepackage{amsmath}
\usepackage{amssymb}
\usepackage{xcolor}
\usepackage{hyperref}

% Title slide info
\title[Lojack and Auto Theft]{Replication and Alternative Identification Strategy}
\subtitle{Deterrence and Geographical Externalities in Auto Theft (Gonzalez-Navarro 2013)}
\author{Manuel Zerobin}
\date{\today}

\begin{document}

% Title slide
\frame{\titlepage}


% ============================================================
% SLIDE 1: RESEARCH QUESTION & DATA
% ============================================================
\begin{frame}[t]
\frametitle{Research Question \& Data}

\textbf{Research Question:}

\begin{itemize}
  \item What is the effect of Lojack (stolen vehicle recovery technology) on auto theft rates?
  \item \textbf{Central motivation:} Does technological deterrence reduce crime, or does it simply displace theft geographically?
  \item \textbf{Policy implication:} Should crime control be centralized (to internalize spillovers) or can local enforcement suffice?
\end{itemize}

\vspace{0.2cm}

\textbf{Data:}

\begin{itemize}
  \item Source: Mexican auto insurance claims database (AMIS), 1999--2004
  \item Coverage: All reported vehicle thefts in 31 Mexican states
  \item Unit of observation: state $\times$ model $\times$ vintage $\times$ year
  \item Total observations: $N = 16,764$ cells
  \item Treatment: Lojack installed in select Ford models in Jalisco (2001) and Mexico City metro (2002)
\end{itemize}

\end{frame}

% ============================================================
% SLIDE 2: METHODOLOGY & IDENTIFICATION STRATEGY
% ============================================================
\begin{frame}[t]
\frametitle{Methodology \& Identification Strategy}

Paper explicitly rejects DiD due likely spillovers and thus contaminated controls

\vspace{0.1cm}

\textbf{Interrupted Time Series (ITS) Strategy:}

\begin{itemize}
  \item Source of identification: \textbf{within-unit} changes over time
  \item Each unit serves as its own counterfactual
  \item Allows estimation of spillovers: uses all units to identify external effects
  \item Allows for fixed effects (state $\times$ model) and time trends
  \item Estimated using negative binomial regression (positive \& count data)
\end{itemize}

\vspace{0.1cm}

{\small \textbf{Key Identifying Assumption:} Lojack introduction uncorrelated with changes in theft risk \textit{beyond} state-specific trends.}

\vspace{0.1cm}

\textbf{Replication with Callaway St'Anna (2021) estimator:} can be found on \href{https://github.com/m0byn/Paper-Replication}{\textcolor{blue}{Github}} 

\end{frame}

% ============================================================
% SLIDE: ORIGINAL PAPER – MAIN RESULTS
% ============================================================
\begin{frame}[t]
\frametitle{Main Results}

\textbf{1. Deterrence Effect on Lojack-Equipped Vehicles}

\begin{itemize}
  \item Lojack-equipped vehicles in Lojack states: $\approx -0.663$ (SE $\approx 0.018$) $\Rightarrow$ \textbf{$\approx 48\%$ reduction} in theft risk 
\end{itemize}

\vspace{0.3cm}

\textbf{2. Geographical Spillovers (Externalities)}

\begin{itemize}
  \item Lojack models in non-Lojack states (displacement): $\approx 0.417$ (SE $\approx 0.075$) $\Rightarrow$ \textbf{$\approx 52\%$ increase} in thefts
\end{itemize}

\vspace{0.3cm}

\textbf{3. Net Crime Effect and Policy Implication}

\begin{itemize}
  \item Reduction in thefts in Lojack states exceeds increase in non-Lojack states
  \item Conclusion: Lojack generates substantial deterrence with incomplete displacement
  \item Policy implication: technologies can reduce overall crime, but spatial externalities justify centralized coordination
\end{itemize}

\end{frame}


% ============================================================
% SLIDE 3: CS ALTERNATIVE & RESULTS
% ============================================================
\begin{frame}[t]
\frametitle{Conclusion: Identification Strategies \& Robustness}

\textbf{Main Takeaways:}

\begin{enumerate}
  \item \textbf{Replication:} Original results confirmed; small data-handling differences explain SE variations
  
  \vspace{0.25cm}
  
  \item \textbf{ITS vs. CS-DiD:} Two fundamentally different identification strategies yield similar deterrence magnitudes ($-0.66$ vs. $-0.77$), suggesting effect is robust across methodological approaches
  
  \vspace{0.25cm}
  
  \item \textbf{Pre-trends violation in CS:} Significant positive estimate at event time $t=-2$ empirically validates paper's concern that DiD assumptions fail in this spatial setting
  
  \vspace{0.25cm}
  
  \item \textbf{Spillover effects:} CS framework cannot estimate these— that is why paper designed identification to handle spatial externalities
\end{enumerate}

\vspace{0.1cm}

\textbf{Interpretation:}
\begin{itemize}
  \item The similarity of point estimates across ITS and CS does \textit{not validate} either method
  \item CS robustness check supports the paper's concerns about cross-unit comparisons in the presence of spatial effects
\end{itemize}

\end{frame}

% ============================================================
% SLIDE 4: LIMITATIONS
% ============================================================
\begin{frame}[t]
\frametitle{Limitations}

\textbf{Limitations of ITS Approach:}

\begin{enumerate}
  \item \textbf{Trend specification:} Assumes state-specific \textit{quadratic} trends are sufficient
  \begin{itemize}
    \item What if true trends are higher-order or nonlinear?
    \item Misspecification would bias treatment effects
  \end{itemize}
  
  \item \textbf{Unobserved confounds:} Assumes no time-varying state-level shocks correlated with treatment
  \begin{itemize}
    \item Police crackdowns, economic shifts, gang activity changes unobserved
    \item Unobservable factors could drive theft changes independent of Lojack
  \end{itemize}
  
  \item \textbf{Cannot estimate spillovers precisely:} Spillover coefficients rely on model assumptions
  \begin{itemize}
    \item Identifying spillovers requires modeling uncontaminated ``nearby'' vs.\ ``distant'' states
    \item Results sensitive to distance cutoffs (tercile definitions)
  \end{itemize}
  
  \item \textbf{Exposure measurement:} Controls for sales at \textit{time of sale}, not current vehicle stock
  \begin{itemize}
    \item Vehicles sold in 1999 still at risk in 2004; data doesn't track cumulative stock
    \item May overstate or understate theft risk in older vintages
  \end{itemize}
\end{enumerate}

\end{frame}

% ============================================================
% Thank you!
% ============================================================

\begin{frame}
\center
\textbf{Thank you!}
\end{frame}

% ============================================================
% BACKUP 1: Data Preparation
% ============================================================
\appendix


% ============================================================
% SLIDE 1: PAPER OVERVIEW
% ============================================================
\begin{frame}[t]
\frametitle{Paper Overview: Research Question \& Data}

\begin{columns}[T]
\column{0.5\textwidth}
\textbf{Research Question:}
\begin{itemize}
  \item Effect of Lojack (stolen vehicle recovery system) on auto theft deterrence
  \item Main finding: 48\% reduction in thefts for equipped vehicles
  \item Geographical spillovers: 18\% of reduction displaced to non-Lojack states
\end{itemize}

\column{0.5\textwidth}
\textbf{Data \& Context:}
\begin{itemize}
  \item Mexican auto insurance claims (AMIS), 1999--2004
  \item Staggered introduction: Jalisco (2001), 3 metro areas (2002)
  \item Unit: state $\times$ model $\times$ vintage $\times$ year
  \item $N = 16,764$ observations
\end{itemize}
\end{columns}

\vspace{0.5cm}

\textbf{Identification Strategy (Original Paper):}
\begin{itemize}
  \item Interrupted time series
  \item Within-unit comparisons: each unit vs. own pre-treatment trajectory
  \item State-specific linear \& quadratic time trends
  \item Negative binomial with exposure (sales)
\end{itemize}

\note{The paper explicitly rejects DiD due to spatial externalities}
\end{frame}

% ============================================================
% SLIDE 2: REPLICATION & DATA ISSUE
% ============================================================
\begin{frame}[t]
\frametitle{Replication Results \& Data Discrepancy}

\textbf{Replication Status:}
\begin{itemize}
  \item Main results replicated successfully
  \item Standard errors differ slightly
\end{itemize}

\vspace{0.2cm}

\textbf{Data Issue Identified:}
\begin{itemize}
  \item Replicated \& original $N = 16,764$
  \item Replicated $\Delta_{LJmodel=1} = 65$
  \item Discrepancy: slight exclusion/inclusion of edge cases
  \item Impact: point estimates identical, minor changes in SE
\end{itemize}

\end{frame}

% ============================================================
% SLIDE 3: CALLAWAY & SANT'ANNA MOTIVATION
% ============================================================
\begin{frame}[t]
\frametitle{Why Implement Callaway \& Sant'Anna (2021)?}

\textbf{Alternative Identification Strategy:}

\begin{itemize}
  \item Original approach: Interrupted time series (within-unit, flexible trends)
  \item CS approach: Staggered DiD (cross-unit, parallel trends)
  \item \textbf{Purpose:} Assess whether deterrence effect is robust across identification strategies
\end{itemize}

\vspace{0.3cm}

\textbf{Important:}
\begin{itemize}
  \item Paper explicitly rejects DiD due to spatial externalities
  \item CS treats contaminated units (Lojack models in non-Lojack states) as controls
  \item These units \textit{experience spillover effects} ($+52\%$ increase in thefts)
  \item \textbf{This is not validation of paper's identification, but alternative exploration}
\end{itemize}

\vspace{0.5cm}

\textbf{Implementation:}
\begin{itemize}
  \item Unit: model $\times$ state (aggregated from model $\times$ state $\times$ vintage)
  \item Outcome: $\log(\text{total thefts} + 1)$ at model-state-year level
  \item Treated cohorts: Jalisco 2001 (9 units), other metros 2002 (25 units)
  \item Control group: Never-treated model-state pairs (1,371 units)
\end{itemize}

\end{frame}

% ============================================================
% SLIDE 4: CS MAIN RESULTS
% ============================================================
\begin{frame}[t]
\frametitle{Callaway \& Sant'Anna Estimation Results}

\begin{columns}[T]
\column{0.5\textwidth}
\textbf{Overall ATT (Simple Aggregation):}

\begin{table}[!h]
\small
\begin{tabular}{lcc}
\toprule
Method & Coef & SE \\
\midrule
ITS (Original) & $-0.663$ & 0.018 \\
CS (DiD) & $-0.772$ & 0.285 \\
\bottomrule
\end{tabular}
\end{table}

\vspace{0.3cm}
\textit{Interpretation:} 48.5\% vs. 53.8\% reduction—similar in magnitude but CS has wider CI.

\column{0.5\textwidth}
\textbf{Parallel Trends Test:}

\begin{table}[!h]
\small
\begin{tabular}{lcc}
\toprule
Event Time & ATT & SE \\
\midrule
$t = -2$ & $+0.503$ & 0.094 \\
$t = -1$ & $+0.245$ & 0.278 \\
$t = 0$ & $-0.103$ & 0.232 \\
$t = +1$ & $-0.789$ & 0.283 \\
\bottomrule
\end{tabular}
\caption{\small Pre-trends violated at $t = -2$ ($p < 0.001$)}
\end{table}
\end{columns}

\vspace{0.5cm}

\textbf{Key Findings:}
\begin{itemize}
  \item CS yields deterrence magnitude \textit{similar} to ITS but with larger uncertainty
  \item \textcolor{red}{Significant pre-trend at $t = -2$:} parallel trends assumption violated
  \item Event study shows large post-treatment effects consistent with deterrence
  \item \textbf{Conclusion:} Pre-trends empirically validate paper's methodological choice
\end{itemize}

\end{frame}

% ============================================================
% SLIDE 5: CONCLUSION
% ============================================================
\begin{frame}[t]
\frametitle{Conclusion: Identification Strategies \& Robustness}

\textbf{Main Takeaways:}

\begin{enumerate}
  \item \textbf{Replication:} Original results confirmed; small data-handling differences explain SE variations
  
  \vspace{0.3cm}
  
  \item \textbf{ITS vs. DiD:} Two different identification strategies yield similar deterrence magnitudes ($-0.66$ vs. $-0.77$)
  
  \vspace{0.3cm}
  
  \item \textbf{Pre-trends violation in CS:} Significant pre-treatment dynamics validate paper's \textit{a priori} concern that DiD assumptions fail in this setting
  
  \vspace{0.3cm}
  
  \item \textbf{Spillover effects:} CS framework cannot estimate these—emphasizes why paper designed identification to handle spatial externalities
\end{enumerate}

\end{frame}

% ============================================================
% BACKUP: Empirical Specification
% ============================================================

\begin{frame}
\frametitle{Empirical Specification}
\textbf{Empirical Specification:}

\[
E[\text{Thefts}_{smvt}] = S_{smvt} \exp(\alpha_{sm} + f_{st} + \beta_{\text{age}} + \gamma_1 LJ + \gamma_2 \text{NLJS\_LJM} + \ldots)
\]

\begin{itemize}
  \item $\alpha_{sm}$: state $\times$ model fixed effects (absorb cross-sectional heterogeneity)
  \item $f_{st}$: state-specific \textbf{quadratic} time trends (flexible within-unit counterfactual)
  \item $S_{smvt}$: stock of vehicles (exposure); estimated via negative binomial
\end{itemize}
\end{frame}

\begin{frame}[t, fragile]
\frametitle{Data Preparation \& Cohort Structure}

\textbf{CS Panel Construction:}
\begin{itemize}
  \item Aggregate from model $\times$ state $\times$ vintage $\times$ year to model $\times$ state $\times$ year
  \item Define treatment cohort: first year model enters Lojack program in state
  \item Jalisco 2001: 9 model-state units, 52 observations
  \item Mexico City metro 2002: 25 model-state units, 145 observations
  \item Never-treated: 1,371 model-state units across 31 states
\end{itemize}

\vspace{0.4cm}

\textbf{Outcome Variable:}
\begin{itemize}
  \item $\text{outcome} = \log(\text{total thefts} + 1)$ at model-state-year level
  \item ITS used counts; CS uses log scale for computational stability
  \item Allows comparison across aggregation levels
\end{itemize}

\vspace{0.4cm}

\textbf{Why Aggregation Necessary:}
\begin{itemize}
  \item ID-level CS fails: ``panel2cs only for 2 periods'' error
  \item 1 treated unit in 2001, 3 in 2002 too small for DiD inference
  \item Model-state aggregation increases treated cohort sizes
\end{itemize}

\end{frame}

% ============================================================
% BACKUP 2: Event Study Plot
% ============================================================
\begin{frame}[t]
\frametitle{Event Study: Dynamic Treatment Effects}

\begin{center}
\includegraphics[width=0.8\textwidth]{../Output/Plots/event_study_callaway_santanna_model_state.png}
\end{center}

\end{frame}

% ============================================================
% BACKUP 4: Full Model Specification (ITS Original)
% ============================================================
\begin{frame}[t, fragile]
\frametitle{Original ITS Specification (Table 2, Col. 3)}

\textbf{Estimating Equation:}
\[
E[\text{Thefts}_{smvt} | S_{smv}] = S_{smv} \exp(\alpha_{sm} + f_{st} + \beta_{\text{age}} + \gamma_1 LJ_{smv} + \gamma_2 \text{NLJM\_LJS} + \ldots)
\]

\vspace{0.3cm}

\begin{table}[!h]
\small
\begin{tabular}{lcc}
\toprule
Variable & Coef & SE \\
\midrule
$LJ$ (Deterrence) & $-0.663^{***}$ & (0.018) \\
NLJM\_LJS\_After (Within-state) & $-0.083$ & (0.066) \\
NLJS\_LJM\_After (Geographic) & $0.417^{***}$ & (0.075) \\
NLJS\_NLJM\_After (Cross-model) & $0.049$ & (0.056) \\
\bottomrule
\multicolumn{3}{l}{\small$^{***}p < 0.01$; Clustered SE at state level}
\end{tabular}
\end{table}

\vspace{0.3cm}

\textbf{Controls:}
\begin{itemize}
  \item State $\times$ model fixed effects
  \item State-specific quadratic time trends
  \item Age dummies (0, 1, 2, 3+ years)
  \item Exposure: $\log(\text{sales})$
\end{itemize}

\end{frame}

% ============================================================
% BACKUP 5: Parallel Trends Detail
% ============================================================
\begin{frame}[t]
\frametitle{Parallel Trends Test: Full Results}

\begin{table}[!h]
\small
\begin{tabular}{lcccc}
\toprule
Period & ATT & SE & Z-stat & P-value \\
\midrule
$t = -2$ & $0.503$ & 0.094 & 5.377 & $< 0.001$ \\
$t = -1$ & $0.245$ & 0.278 & 0.879 & 0.379 \\
$t = 0$ & $-0.103$ & 0.232 & -0.444 & 0.657 \\
$t = +1$ & $-0.789$ & 0.283 & -2.791 & 0.005 \\
$t = +2$ & $-1.268$ & 0.398 & -3.189 & 0.001 \\
$t = +3$ & $-1.371$ & 0.061 & -22.541 & $< 0.001$ \\
\bottomrule
\end{tabular}
\caption{Dynamic ATT by Event Time (CS Estimator)}
\end{table}

\vspace{0.3cm}

\textbf{Implication:}
\begin{itemize}
  \item 1 of 2 pre-treatment periods significant at $p < 0.05$
  \item Overall pre-trend test p-value: 0.015 (rejects null of parallel trends)
  \item Confirms paper's concern: DiD assumptions fail in this data
\end{itemize}

\end{frame}

% ============================================================
% BACKUP 7: Methodological Discussion
% ============================================================
\begin{frame}[t]
\frametitle{Identification Strategies: ITS vs. DiD}

\begin{columns}[T]
\column{0.5\textwidth}
\textbf{Interrupted Time Series (ITS):}
\begin{itemize}
  \item Source of variation: within-unit over time
  \item Comparison: each unit to its own pre-trend
  \item Assumption: treatment uncorrelated with state-specific trends
  \item Allows: flexible trend adjustment
\end{itemize}

\column{0.5\textwidth}
\textbf{Difference-in-Differences (DiD):}
\begin{itemize}
  \item Source of variation: cross-unit
  \item Comparison: treated vs. never-treated
  \item Assumption: parallel trends
  \item Requires: uncontaminated controls
\end{itemize}
\end{columns}

\vspace{0.5cm}

\textbf{This Application:}
\begin{itemize}
  \item Spatial externalities violate DiD (controls are contaminated)
  \item ITS appropriate because: within-unit identification avoids cross-unit contamination
  \item CS implementation reveals: pre-trends exist, confirming paper's choice
\end{itemize}

\end{frame}

% ============================================================
% BACKUP 8: Limitations & Future Work
% ============================================================
\begin{frame}[t]
\frametitle{Limitations \& Extensions}

\textbf{CS Implementation Limitations:}
\begin{enumerate}
  \item \textbf{Contaminated control group:} Lojack models in non-Lojack states (experience spillover)
  \item \textbf{Parallel trends violated:} Significant pre-trend at $t = -2$
  \item \textbf{Model aggregation:} Loses within-state model-specific dynamics
  \item \textbf{Outcome transformation:} Log scale differs from count-based ITS
\end{enumerate}

\vspace{0.4cm}

\textbf{Potential Extensions:}
\begin{itemize}
  \item Restrict never-treated to non-Lojack models only (drop spillover-affected units)
  \item Implement honest DiD (Roth, 2022) accounting for pre-trend violations
  \item Use alternative estimators (Callaway et al., 2021; Sun \& Abraham, 2021) robust to heterogeneity
  \item Examine sensitivity to specification choices (bandwidth, trend order)
\end{itemize}

\end{frame}

\end{document}
